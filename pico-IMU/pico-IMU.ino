...
#define MOT_TOP_LEFT 13
#define MOT_TOP_RIGHT 18
#define MOT_BOTTOM_LEFT 1
#define MOT_BOTTOM_RIGHT 28
#define Green_LED 21
#define Red_LED 20
...
float outside_pid_Kp_roll = 0.85;
float outside_pid_Ki_roll = 0.065;
float outside_pid_Kd_roll = 0.06;
float inside_pid_Kp_roll = 0.6;
float inside_pid_Ki_roll = 0.0;
float inside_pid_Kd_roll = 0.0;
...
float outside_pid_Kp_yaw = 3.1;
float outside_pid_Ki_yaw = 0.01;
float outside_pid_Kd_yaw = 0.3;
...
void setup(){
    ...
    analogWriteRange(1000);//该函数用于改变PWM的值(占空比), 1000对应100%。
...
void loop(){
    ...
    // angle_roll = getAngle(angle_roll_acc, gyro_roll, dt);hold_throttle
    ...
    //停止电机：油门低，偏航左
    if(start == 2 && receiver_input_channel_3 < 1100 && receiver_input_channel_4 < 1350)
    {
      start = 0;
      digitalWrite(Red_LED, HIGH);
      digitalWrite(Green_LED, LOW);
    ...
    if(receiver_input_channel_1 > 1510)pid_roll_setpoint = 0.3*(receiver_input_channel_1 - 1510);
    else if(receiver_input_channel_1 < 1490)pid_roll_setpoint = 0.3*(receiver_input_channel_1 - 1490);
    ...
    if(receiver_input_channel_2 > 1510)pid_pitch_setpoint = 0.3*(receiver_input_channel_2 - 1510);
    else if(receiver_input_channel_2 < 1490)pid_pitch_setpoint = 0.3*(receiver_input_channel_2 - 1490);
    pid_pitch_setpoint -= pitch_level_adjust;
    pid_pitch_setpoint /= 1.0;
    // pid_yaw_setpoint = 0;
    if(receiver_input_channel_3 > 1100){ //Do not yaw when turning off the motors.
        if(receiver_input_channel_4 > 1510)
        {
          // pid_yaw_setpoint = (receiver_input_channel_4 - 1510);
          // pid_yaw_setpoint /= 12.0;
          pid_yaw_setpoint += 0.1;
        }
    ...
    if(start == 2){
        ...
        esc_1 = throttle + pid_output_pitch - pid_output_roll + 0.4*pid_output_yaw; //Calculate the pulse for esc 1 (front-right - CCW)
        esc_2 = throttle - pid_output_pitch - pid_output_roll - 0.4*pid_output_yaw; //Calculate the pulse for esc 2 (rear-right - CW)
        esc_3 = throttle - pid_output_pitch + pid_output_roll + 0.4*pid_output_yaw; //Calculate the pulse for esc 3 (rear-left - CCW)
        esc_4 = throttle + pid_output_pitch + pid_output_roll - 0.4*pid_output_yaw; //Calculate the pulse for esc 4 (front-left - CW)
...
void calculate_pid_roll()
{
  ...
  outside_pid_output_roll=outside_Kp_output_roll+outside_Ki_output_roll+outside_pid_Kd_roll*(outside_pid_angle_err_roll-last_outside_pid_angle_err_roll)/dt;
  ...
  pid_output_roll=inside_Kp_output_roll+inside_Ki_output_roll+inside_pid_Kd_roll*(inside_pid_angular_err_roll-last_inside_pid_angular_err_roll)/dt;
...
void calculate_pid_pitch()
{
  ...
  //外环PID输出=外环PID_P项+外环PID_I项
  outside_pid_output_pitch=outside_Kp_output_pitch+outside_Ki_output_pitch+outside_pid_Kd_pitch*(outside_pid_angle_err_pitch-last_outside_pid_angle_err_pitch)/dt;
  ...
  //内环PID_D项 = 内环Kd * 当前角速度的微分
  //内环PID输出 = 内环PID_P项 + 内环PID_I项 + 内环PID_D项
  pid_output_pitch=inside_Kp_output_pitch+inside_Ki_output_pitch+inside_pid_Kd_pitch*(inside_pid_angular_err_pitch-last_inside_pid_angular_err_pitch)/dt;
...
void calculate_pid_yaw()
{
  ...
  //外环PID输出=外环PID_P项+外环PID_I项
  pid_output_yaw=outside_Kp_output_yaw+outside_Ki_output_yaw+outside_pid_Kd_yaw*(outside_pid_angle_err_yaw-last_outside_pid_angle_err_yaw)/dt;
